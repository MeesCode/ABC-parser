...
TEST(empty_input) {
    ...
    ASSERT_EQ(result, 0);
    ASSERT_EQ(g_sheet.note_count, 0);
    return 1;
}
...
TEST(single_note_each) {
    int result = abc_parse(&g_sheet, "K:C\nC D E F G A B");
    ASSERT_EQ(result, 0);
    ASSERT_EQ(g_sheet.note_count, 7);

    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_EQ(n->note_name, NOTE_C); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->note_name, NOTE_D); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->note_name, NOTE_E); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->note_name, NOTE_F); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->note_name, NOTE_G); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->note_name, NOTE_A); n = note_next(&g_sheet, n);
...
TEST(lowercase_notes) {
    ...
    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_EQ(n->note_name, NOTE_C);
    ASSERT_EQ(n->octave, 5);  // lowercase c is octave 5
    n = note_next(&g_sheet, n);
    ASSERT_EQ(n->note_name, NOTE_D);
...
TEST(octave_modifier_up) {
    ...
    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_EQ(n->octave, 5); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->octave, 6); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->octave, 6);  // clamped to max 6
    return 1;
}
...
TEST(octave_modifier_down) {
    ...
    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_EQ(n->octave, 4); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->octave, 3); n = note_next(&g_sheet, n);
...
TEST(sharp) {
    ...
    ASSERT_EQ(g_sheet.note_count, 1);
    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_EQ(n->note_name, NOTE_F);
    ASSERT_EQ(n->accidental, ACC_SHARP);
    return 1;
}
...
TEST(flat) {
    ...
    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_EQ(n->note_name, NOTE_B);
    ASSERT_EQ(n->accidental, ACC_FLAT);
    return 1;
}
...
TEST(natural) {
    ...
    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_EQ(n->note_name, NOTE_F);
    ASSERT_EQ(n->accidental, ACC_NONE);  // natural cancels to none
    return 1;
}
...
TEST(double_sharp) {
    ...
    ASSERT_EQ(result, 0);
    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_EQ(n->accidental, ACC_DOUBLE_SHARP);
    return 1;
}
...
TEST(double_flat) {
    ...
    ASSERT_EQ(result, 0);
    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_EQ(n->accidental, ACC_DOUBLE_FLAT);
    return 1;
}
...
TEST(accidental_persists_in_bar) {
    ...
    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_EQ(n->accidental, ACC_SHARP); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->accidental, ACC_SHARP); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->accidental, ACC_SHARP);
    return 1;
}
...
TEST(accidental_resets_at_bar) {
    ...
    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_EQ(n->accidental, ACC_SHARP); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->accidental, ACC_NONE);  // reset after bar
    return 1;
}
...
TEST(rest_lowercase) {
    ...
    ASSERT_EQ(g_sheet.note_count, 1);
    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_EQ(n->note_name, NOTE_REST);
    ASSERT_EQ(n->frequency_x10, 0);
    return 1;
}
...
TEST(key_a_minor) {
    ...
    ASSERT_EQ(result, 0);
    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_EQ(n->accidental, ACC_NONE); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->accidental, ACC_NONE);
    return 1;
}
...
TEST(simple_repeat) {
    int result = abc_parse(&g_sheet, "K:C\n|:C D:|");
    ASSERT_EQ(result, 0);
    ASSERT_EQ(g_sheet.note_count, 4);  // C D C D
...
TEST(notes_before_repeat) {
    int result = abc_parse(&g_sheet, "K:C\nA B |:C D:|");
    ASSERT_EQ(result, 0);
    ASSERT_EQ(g_sheet.note_count, 6);  // A B C D C D

    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_EQ(n->note_name, NOTE_A); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->note_name, NOTE_B); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->note_name, NOTE_C); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->note_name, NOTE_D); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->note_name, NOTE_C); n = note_next(&g_sheet, n);
    ASSERT_EQ(n->note_name, NOTE_D);
...
TEST(notes_after_repeat) {
    int result = abc_parse(&g_sheet, "K:C\n|:C D:| E F");
    ASSERT_EQ(result, 0);
    ASSERT_EQ(g_sheet.note_count, 6);  // C D C D E F
    return 1;
}
...
TEST(frequency_middle_c) {
    ...
    ASSERT_EQ(result, 0);
    struct note *n = sheet_first_note(&g_sheet);
    ASSERT_FLOAT_EQ(n->frequency_x10 / 10.0f, 261.6f, 1.0f);
    return 1;
}
...
TEST(ignore_decorations) {
    // Parser should skip [], (), {}, etc.
    int result = abc_parse(&g_sheet, "K:C\n(C D) [E F]");
    ASSERT_EQ(result, 0);
    ASSERT_EQ(g_sheet.note_count, 4);
    return 1;
...
TEST(greensleeves_excerpt) {
    ...
    ASSERT_EQ(g_sheet.note_count, 6);
    return 1;
}
...
TEST(total_duration) {
    ...
    ASSERT_EQ(g_sheet.total_duration_ms, 2000);
    return 1;
}
